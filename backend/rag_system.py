...
class RAGSystem:
    ...
    def query(self, query: str, session_id: Optional[str] = None) -> Tuple[str, List[str]]:
        """
        Process a user query using the RAG system with direct search.
        ...
        course_list = "\n".join([f"- {title}" for title in course_analytics["course_titles"]])
        ...
        if not search_results.is_empty():
            # Iterate over documents and metadata together
            for doc, meta in zip(search_results.documents, search_results.metadata):
                # Extract source information from metadata
                course_title = meta.get('course_title', 'Unknown Course')
                lesson_number = meta.get('lesson_number', None)

                # Format source with lesson number
                if lesson_number is not None:
                    source = f"{course_title} - Lesson {lesson_number}"
                else:
                    source = course_title
        ...
        # Create prompt with context including course metadata
        prompt = f"""You have access to {course_analytics["total_courses"]} courses:
{course_list}

Based on the following course materials, answer the question.
...
Provide a clear, concise answer based on the materials above. If the question is about course counts or course lists, use the course information provided at the top."""
        ...
        # Generate response using AI without tools
